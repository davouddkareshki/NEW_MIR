...
class ClusteringDataLoader:
    ...
    def create_train_data(self):
        ...
        for i,label in enumerate(df['genres']) :
            ...
            flag = 0 
            for j in range(4) :  
                if X[i][j] == '' or X[i][j] == None or X[i][j] == [] : 
                    flag = 1
            #NOTE:uncomment below line for fasttext 
            if flag : 
                continue
            if len(label) > 0 :
                text = label[0] 
                clustering_X.append(X[i])
                clustering_y.append(text)     
            
            y.append(text)
        y = label_encoder.fit_transform(y)
        clustering_y = label_encoder.fit_transform(clustering_y)

        return np.array(clustering_X), np.array(clustering_y)

if __name__ == "__main__":
    ft_data_loader = ClusteringDataLoader('IMDB_crawled.json')
    ...
    for i in tqdm.tqdm(range(int(len(X)))) : 
        obj = {}
        real_idx = 0
        flag = 0
        for idx in range(len(X[i])) : 
            khar = ft_model.get_query_embedding(X[i][idx]) 
            if len(khar) == 0 : 
                flag = 1 
            else : 
                for val in khar :
                    obj[real_idx] = float(val) 
                    real_idx += 1
        obj['label'] = y[i]
        if flag == 0 : 
            clustering_train_data.append(obj)
    clustering_train_data = pd.DataFrame(clustering_train_data) 
    clustering_train_data.to_csv('clustering_train_data.csv', index=False)
