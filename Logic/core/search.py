...
class SearchEngine:
       ...
       # print(query_terms)
        ...
        for field in weights:
            ...
            for tier in self.tiered_index[field].keys():
                ...
                scorer = Scorer(self.tiered_index[field][tier], 1005, doc_lentgh_index_tier) 
                if method == 'OkapiBM25' :
    ...
    def find_scores_with_safe_ranking(self, query, method, weights):
        ...
        scores = {} 
        for field in weights:
            scorer = Scorer(self.document_indexes[field],1005,self.document_lengths_index[field]) 
            if method == 'OkapiBM25' :
